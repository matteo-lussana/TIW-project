...
public class CategoryDAO {
	...
	public List<CategoryClass> findAllCategories() throws SQLException{
		...
		try(PreparedStatement pstatement = connection.prepareStatement("SELECT * FROM category ORDER BY code ASC");){
			try(ResultSet result = pstatement.executeQuery();){
				while(result.next()) {
					...
					if((cat.getCode() % 10 == 9 && cat.getFather()!=0)) categories.get(binaryResearch(cat.getFather(), categories)).setIsComplete(true);
	 ...
	 * @throws SQLException
	 */
	public void createCategory(long newChildCode, String name, long fatherCode) throws SQLException{
		String query = "INSERT into category (code, name, father) VALUES (?, ?, ?)";
		try(PreparedStatement pstatement = connection.prepareStatement(query);){
			pstatement.setLong(1, newChildCode);
	...
	public List<CategoryClass> findFamily(long ancestorCode, List<CategoryClass> categories){
		...
		for(int i=ancestorPos+1; i<categories.size(); i++) {
			...
			if(ancestorCode == ancestorOfI) family.add(categories.get(i));
		}
	...
	/*public List<CategoryClass> findNoCopyHereCategories(List<CategoryClass> familyToCopy, List<CategoryClass> categories){
		...
		int familyDepth = computeDescendantsNum(familyToCopy.get(0).getCode(), familyToCopy);
	...
	public List<CategoryClass> findNoCopyCategories(List<CategoryClass> categories){
		...
		for(CategoryClass cat : categories) {
			int descendantsNum = computeDescendantsNum(cat.getCode(), categories);
	...
	/**
	 * This method changes the name of the category in the database
	 * @param code of the category
	 * @param name: the new one
	 * @throws SQLException 
	 */
	public void editName(long code, String name) throws SQLException{
		String query = "UPDATE category SET name = ? WHERE code = ?";
		
		try(PreparedStatement pstatement = connection.prepareStatement(query);){
			pstatement.setString(1, name);
			pstatement.setLong(2, code);
			pstatement.executeUpdate();
		}
